...
def enviar_mensaje_whatsapp(telefono_destino, texto, link_boton=None):
    ...
    if link_boton:
        data = {
            ...
            "interactive": {
                "type": "cta_url", "body": { "text": texto },
                "action": {
                    "name": "cta_url",
                    "parameters": { "display_text": "Hablar con asesor", "url": link_boton }
                }
            }
        }
    else:
        ...
        if para_derivar:
            for telefono, historial_str in para_derivar:
                try:
                    ...
                    execute_db_query("""
                        INSERT INTO chats_derivados (telefono, vendedor, historial, fecha) VALUES (%s, %s, %s, %s)
                    ...
                    aviso_asesor = (
                        ...
                        f"👉 *Acción requerida:* Por favor, revisa el panel de 'Chats Pendientes' en el sistema y contactalo."
                    )
                ...
                except Exception as inner_e: print(f"Fallo derivando chat {telefono}: {inner_e}")
    except Exception as e: print(f"Error general en rutinas de tiempo: {e}")
...
NÚMERO DEL CLIENTE ACTUAL: +{telefono_cliente_completo} (Úsalo obligatoriamente cuando debas agendar un contacto)."""
    else:
